...
不要线性阅读。选一个具体的操作（一个系统调用、一个中断、一个数据包），用`grep`或IDE在源码中追踪它的路径。`include/linux/`和`kernel/`下的头文件定义了核心数据结构——先读这些。`Documentation/`目录（以及`https://www.kernel.org/doc/html/`）解释了设计原理。并且先自己动手编译内核——这会让源码树变得具体可触。
